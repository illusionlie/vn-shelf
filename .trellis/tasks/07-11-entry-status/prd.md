...
- [ ] 条目支持游玩状态字段：API 可写可读、有枚举校验；前端可编辑、卡片可视化、可筛选；详情弹窗展示。
- [ ] 导入导出双向兼容旧备份文件。
- [ ] 枚举设计与 VNDB label 映射规则文档化（本 PRD「决策记录」+ 子任务 design.md），供未来 ulist 导入任务直接引用。
- [ ] `npm run lint` 与 `npm run test` 通过；AGENTS.md 数据结构文档同步更新。
